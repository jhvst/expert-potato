...
const airports = {
  ...
  XIY: {
   ...
   LGW:  {
      airport: "[Duplicate] London Gatwick",
      city: "London"
  },
   PUY:  {
   ...
   MUC:  {
      airport: "Flughafen München-Riem",
      city: "Munich"
  },
   KTT:  {
   ...
   KRK:  {
      airport: "John Paul II International Airport Kraków-Balice Airport",
      city: "Kraków"
  },
   FUK:  {
...
.on('json',(jsonObj)=>{
  let departureCode = jsonObj.PLAN_DEPARTURE_STATION
  let arrivalCode = jsonObj.PLAN_ARRIVAL_STATION
  jsonObj.departureCity = airports[departureCode].city || ''
  jsonObj.departureAirport = airports[departureCode].airport || ''
  jsonObj.arrivalCity = airports[arrivalCode].city || ''
  jsonObj.arrivalAirport = airports[arrivalCode].airport || ''
  
  funkyJsonArr.push(JSON.stringify(jsonObj))
})
.on('done',(error)=>{
